...
public class ByteDetective {
    ...
    public static void readAllBytes() {
        ...
        try(FileInputStream input = new FileInputStream("mystery.bin")){
            ...
            while((bite = input.read()) != -1){
                //       Inside loop:
                System.out.print(bite + " ");  //  Print the byte value
    ...
    public static void displayAsHex() {
        ...
        try(FileInputStream input = new FileInputStream("mystery.bin")){
            ...
            while((bite = input.read()) != -1){
                System.out.print(String.format("%02X ", bite) +" ");
                ; // hex formatting
                count++;
            }
    ...
    public static void attemptAsAscii() {
        ...
        try(FileInputStream input = new FileInputStream("mystery.bin")){
            ...
            while((bite = input.read()) != -1){
                System.out.print((char)bite + " "); // casting as char
                count++;
            }
    ...
    public static void analyzeFile() {
        System.out.println("Implement analyzeFile()");

        // TODO: Initialize tracking variables:
        int count = 0; // int count = 0;
        int min = 256;// int min = 256;  (start higher than any byte)
        int max = -1; // int max = -1;   (start lower than any byte)
        ...
        try(FileInputStream input = new FileInputStream("mystery.bin")){
            ...
            while((bite = input.read()) != -1){
                ...
                if(bite > max){ //Update max if byteValue > max
                    max = bite;
                }
                sum += bite; //Add byteValue to sum

                count++; //Increment count
            }
            // TODO: After loop, calculate and display:

            System.out.println("Total Bytes: " + count); // Total bytes (count)
            System.out.println("Minumum byte value: " + min); // Min value
            System.out.println("Maximum byte value: " + max); // Max value

            // Average (sum / count as a double)
            System.out.println("Average byte value: " + sum/(double)count);
